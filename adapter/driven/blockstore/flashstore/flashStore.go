...
type Store struct {
	...
	buf        []byte
	free       []int
	topics     map[domain.TopicName]map[blockKey]*blockMeta
}
...
func New(dev Device) (*Store, error) {
	...
	s := &Store{
		...
		topics:     make(map[domain.TopicName]map[blockKey]*blockMeta),
	}
...
func (s *Store) scan() error {
	...
	for page := 0; page < s.dev.Pages(); page++ {
		...
		infos = append(infos, pageInfo{
			topic: domain.TopicName(s.buf[offName : offName+nameLen]),
			key: blockKey{
				kind:  driven.BlockKind(s.buf[offKind]),
...
func (s *Store) List(topic domain.TopicName, kind driven.BlockKind) ([]driven.Block, error) {
	...
	for key, meta := range s.topics[topic] {
		...
		blocks = append(blocks, driven.Block{Block: key.block, Size: meta.size})
	}
...
func (s *Store) Append(ref driven.BlockRef, data []byte) (driven.Block, error) {
	...
	if newPages > len(s.free) {
		return driven.Block{}, fmt.Errorf("%w: %d pages needed, %d left", ErrNoSpace, newPages, len(s.free))
	}
	...
	for i, c := range chunks {
		...
		if err := s.markUsed(c.page, c.offset, len(c.data)); err != nil {
			...
			if i > 0 || c.pageIdx < len(meta.pages) {
				...
				s.refresh(meta)
				blocks[key] = meta
				return driven.Block{}, driven.DirtyBlock(err)
			}
			return driven.Block{}, err
		}
	}
	...
	blocks[key] = meta
	return driven.Block{Block: ref.Block, Size: meta.size}, nil
}
...
func (s *Store) claim(ref driven.BlockRef, firstSeq int, count int) ([]int, error) {
...
func (s *Store) Truncate(ref driven.BlockRef, size int64) error {
	...
	if size < 0 || size > meta.size {
		return fmt.Errorf("%w: block %s holds %d bytes, cannot truncate to %d", driven.ErrInvalidSize, ref, meta.size, size)
	}
...
func (s *Store) rewritePage(ref driven.BlockRef, meta *blockMeta, idx int, used int) error {
...
func (s *Store) writeHeader(page int, ref driven.BlockRef, seq uint32) error {
