...
func TestRecoverBlock(t *testing.T) {
	...
	for i, payload := range []string{"dummy1", "dummy2", "dummy3"} {
		valid = append(valid, domain.CreateByteEntry([]byte(payload), domain.Offset(100+i))...)
	}
	...
	tests := []struct {
		...
		{name: "partial entry", content: withTail(domain.CreateByteEntry([]byte("dummy4"), 103)[:15]), wantNext: 103, wantSize: 78, wantTruncated: 15},
...
func TestRecoverBlock_offsetGapIsAnError(t *testing.T) {
	content := append(domain.CreateByteEntry([]byte("dummy1"), 0), domain.CreateByteEntry([]byte("dummy2"), 2)...)
...
func TestReadFile_splitsBatchesAtByteLimit(t *testing.T) {
	...
	var content []byte
	for i := 0; i < 25; i++ {
		content = append(content, domain.CreateByteEntry(payload, domain.Offset(i))...)
	}
...
func TestReadFile_rejectsCorruptEntry(t *testing.T) {
	content := append(domain.CreateByteEntry([]byte("dummy1"), 0), domain.CreateByteEntry([]byte("dummy2"), 1)...)
...
func TestReadFile_rejectsZeroBatchSize(t *testing.T) {
	_, err := readFileContent(t, domain.CreateByteEntry([]byte("dummy1"), 0), 0)
	assert.NotNil(t, err)
}
...
func TestReadFile_hugeBatchSize(t *testing.T) {
	var content []byte
	for i := 0; i < 3; i++ {
		content = append(content, domain.CreateByteEntry([]byte("dummy"), domain.Offset(i))...)
	}
...
func TestReadFile_rejectsUnexpectedFirstOffset(t *testing.T) {
	content := domain.CreateByteEntry([]byte("dummy1"), 7)
...
func TestFindByteOffsetFromAndIncludingOffset(t *testing.T) {
	var content []byte
	for i, payload := range []string{"dummy1", "dummy2", "dummy3"} {
		content = append(content, domain.CreateByteEntry([]byte(payload), domain.Offset(i))...)
	}
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("byteOffset %d and offset %d", test.byteOffsetInput, test.offsetInput), func(t *testing.T) {
			byteOffset, scanned, err := FindByteOffsetFromAndIncludingOffset(store, ref, test.byteOffsetInput, domain.Offset(test.offsetInput))
			assert.Nil(t, err)
