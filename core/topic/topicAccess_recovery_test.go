...
func newTestTopic(t *testing.T, store driven.BlockStore, maxBlockSize int) *Topic {
	t.Helper()
	topic := NewLogTopic(Params{Store: store, TopicName: "t", MaxBlockSize: maxBlockSize})
...
func blockBytes(t *testing.T, store driven.BlockStore, ref driven.BlockRef) []byte {
	t.Helper()
...
func blockSize(t *testing.T, store driven.BlockStore, ref driven.BlockRef) int64 {
	t.Helper()
...
func assertReadsFromEveryOffset(t *testing.T, topic *Topic, n int) {
	...
	for from := 0; from < n; from++ {
		got, err := readAllFrom(topic, domain.Offset(from), 7)
...
func assertIndexMatchesFullScan(t *testing.T, topic *Topic) {
	...
	for _, block := range topic.IndexBlockList {
		logBlock, err := topic.Store.Open(topic.logRef(domain.LogBlock(block)), 0)
		if err != nil {
			t.Fatal(err)
		}
...
func TestTopic_RecoverTornTail(t *testing.T) {
	appendBytes := func(tail []byte) func(*testing.T, driven.BlockStore, driven.BlockRef) {
		return func(t *testing.T, store driven.BlockStore, ref driven.BlockRef) {
			if _, err := store.Append(ref, tail); err != nil {
				t.Fatal(err)
			}
		}
	}
	flipLastByte := func(t *testing.T, store driven.BlockStore, ref driven.BlockRef) {
