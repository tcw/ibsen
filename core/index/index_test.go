...
func TestFindNearestOffset(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			offset := index.FindNearestByteOffset(domain.Offset(test.inputOffset))
			assert.Equal(t, domain.Offset(test.expectedOffset), offset.Offset)
...
func TestHeadEmpty(t *testing.T) {
	...
	head := index.Head()
	assert.Equal(t, domain.Offset(0), head.Offset)
...
func TestHead(t *testing.T) {
	index := createTestIndex(10, 10)
	head := index.Head()
	assert.Equal(t, domain.Offset(90), head.Offset)
...
func createTestIndex(every int, entries int) Index {
	var indexOffests []domain.OffsetFilePtr
	for i := 0; i < entries; i++ {
		indexOffset := domain.OffsetFilePtr{
			Offset:     domain.Offset(i * every),
