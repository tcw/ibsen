...
Known, not yet fixed:
- **Unsent log events**: 26 zerolog events in production code have no `.Msg`/`.Send`, so they log nothing and never exit: 25 `log.Fatal().Err(err)` (16 in `cmd/root.go`, 6 in `api/ibsen.go`, one each in `cmd/ibsenClient.go`, `cmd/ibsenBench.go` and the lease renewer in `access/locking/singleInstanceLock.go`) and the background index failure `log.Warn().Err(err)` in `Topic.Write`. Execution continues after the error.
- **TLS paths**: `api/grpcApi/api-server.go` resolves the cert and key with grpc's `testdata.Path`, which joins relative paths onto grpc's own test directory. Absolute paths work.
- **Shutdown** does not wait for in-flight writes or background indexing before releasing the single-writer lock. Recovery repairs a torn tail on the next start.
