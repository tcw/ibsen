...
func TestFindNearestOffset(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			offset := index.FindNearestByteOffset(common.Offset(test.inputOffset))
			assert.Equal(t, common.Offset(test.expectedOffset), offset.Offset)
...
func TestHeadEmpty(t *testing.T) {
	...
	head := index.Head()
	assert.Equal(t, common.Offset(0), head.Offset)
...
func TestHead(t *testing.T) {
	index := createTestIndex(10, 10)
	head := index.Head()
	assert.Equal(t, common.Offset(90), head.Offset)
...
func createTestIndex(every int, entries int) Index {
	var indexOffests []common.OffsetFilePtr
	for i := 0; i < entries; i++ {
		indexOffset := common.OffsetFilePtr{
			Offset:     common.Offset(i * every),
