...
func TestTopic_Write(t *testing.T) {
	...
	nextOffset, _, truncated, err := ibsLog.RecoverBlock(store, common.LogRef("topic1", 0), 0, blocks[0].Size)
	assert.Nil(t, err)
...
func TestTopic_rejectsInvalidTopicName(t *testing.T) {
	...
	topic := NewLogTopic(common.TopicParams{Store: store, TopicName: "../escaped", MaxBlockSize: 1000})
