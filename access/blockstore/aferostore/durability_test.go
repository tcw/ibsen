...
// newDiskStore returns a store on a real directory, so a durability test is about the
// filesystem rather than about afero's in-memory stand-in.
func newDiskStore(t *testing.T) (*Store, string) {
	t.Helper()
	root := t.TempDir()
	return New(&afero.Afero{Fs: afero.NewOsFs()}, root), root
}

// blockFileOnDisk is the path the adapter keeps a block at, which a durability test reads
// without going through the port.
func blockFileOnDisk(root string, ref common.BlockRef) string {
	return filepath.Join(root, string(ref.Topic), blockFileName(ref))
}
...
func TestDurability_SyncOfUnknownBlock(t *testing.T) {
	...
	if err := store.Sync(common.LogRef("topic", 0)); !errors.Is(err, common.ErrBlockNotFound) {
