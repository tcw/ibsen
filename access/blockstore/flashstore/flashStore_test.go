...
func TestFlashStore_Conformance(t *testing.T) {
	conformance.Run(t, func(t *testing.T) common.BlockStore {
		store, _ := newTestStore(t)
		return store
	})
}

func read(t *testing.T, store *Store, ref common.BlockRef, byteOffset int64) []byte {
	t.Helper()
...
func TestFlashStore_PageTableIsRebuiltFromTheRegion(t *testing.T) {
	...
	want := map[common.BlockRef][]byte{
		common.LogRef("orders", 0):    bytes.Repeat([]byte("a"), 3*store.DataPerPage()+7),
		common.LogRef("orders", 1000): []byte("second block"),
		common.IndexRef("orders", 0):  []byte("index pairs"),
		common.LogRef("payments", 0):  bytes.Repeat([]byte("b"), store.DataPerPage()),
	}
	...
	blocks, err := reopened.List("orders", common.Log)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFlashStore_TopicNameMustFitThePageHeader(t *testing.T) {
	store, _ := newTestStore(t)
	tooLong := common.TopicName(strings.Repeat("t", MaxTopicNameLength+1))
	if _, err := store.CreateTopic(tooLong); !errors.Is(err, common.ErrInvalidTopicName) {
		t.Fatalf("create: err=%v, want ErrInvalidTopicName", err)
	}
	if _, err := store.Append(common.LogRef(tooLong, 0), []byte("x")); !errors.Is(err, common.ErrInvalidTopicName) {
		t.Fatalf("append: err=%v, want ErrInvalidTopicName", err)
	}
	fits := common.TopicName(strings.Repeat("t", MaxTopicNameLength))
	if _, err := store.Append(common.LogRef(fits, 0), []byte("x")); err != nil {
